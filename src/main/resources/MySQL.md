...
1. 表空间
   ...
   - InnoDB 是以页为单位管理存储空间的，我们的聚簇索引（也就是完整的表数据）和其他的二级索引都是以 B+树的形式保存到表空间的，而B+树的节点就是数据页以页的角度看，每个页都又File Header这个部分，其中包含一个属性fil_page_arch_log_no_or_space_id保存页属于哪个表空间。同时每一个页对应着一个页号，这个页号4个字节组成，所以一个表空间最多可以拥有$$2^{32}$$次方个页。按照页默认大小16k来算，那么一个表空间最多可以支持64TB的数据
...
1. redo log
   - redo log又称重做日志文件，用于记录事务操作的变化，记录的是数据修改之后的记录，不管事务是否提交都会记录下来
