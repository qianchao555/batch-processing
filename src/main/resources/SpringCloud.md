...
3. 分布式锁
4. token令牌
   - 当客户端请求页面时，服务器会生成一个token
   - 然后 调用业务接口请求时，把token携带过去，一般放在请求头部。
   - 服务器 判断 token是否存在redis中， 存在表示第一次请求 ，可以继续执行业务，执行业务完成后，最后需要把redis中的token删除。
   - 如果判断token不存在redis中，就表示是重复操作，直接返回重复标记给client，这样就保证了业务代码，不被重复执行
   - ![image-20220414172148460](https://gitee.com/qianchao_repo/pic-typora/raw/master/springcloud_img/image-20220414172148460.png)
