...
通常请情况：Java命令或IDE启动主方法时，JVM程序就会执行，操作系统将其装载入内存，并创建JVM进程，随后启动主线程，主线程会去调用某个类的main方法，因此：这个主线程就是main方法所在
...
并行：两个及以上的线程在同一时刻发生，称为并行。例如两个线程在同一时刻在两个不同的cpu上执行

并发：两个及以上的线程在同一时间段内执行，称为并发。例如两个线程在极短的时间段上分别在同一个cpu上执行
...
任务切换前会保存上一个任务的状态，以便下次切换回这个任务时，可以再加载这个任务的状态。所以任务从保存再到加载的过程就是一次上下文切换，上下文切换会影响多线程的执行速度
...
2. 可运行态(就绪态)Runnable：线程创建后，调用start()方法，此时线程处于可运行线程池中，等待被线程调度选中，获取cpu是使用
   - 包含ready和running状态
   - 运行态Running：线程获得了cpu时间片，执行任务
3. 阻塞态Blocked：线程由于某种原因放弃了cpu的使用权，直到线程进入可运行态，才有机会再次获得cpu时间片进入到运行态
4. Waiting：等待状态
...
JMM规定：线程对共享变量的所以操作都必须在自己的本地内存中进行，不能直接从主内存中读取
...
1. 程序顺序规则：一个线程内，按照代码顺序，书写在前面的操作 happens-before与书写在后面的操作
2. 监视器锁规则：对一个锁的解锁，happens-before于随后对这个锁的解锁
...
为了保证各个处理器的缓存是一致的，实现了**缓存一致性协议MESI**，每个处理器通过嗅探在总线上传播的数据来检查自己缓存的值是不是过期了，当处理器发现自己缓存行对应的内存地址被修改，就会将当前处理器的缓存行设置成无效状态，当处理器对这个数据进行修改操作的时候，会重新从系统内存中把数据读到处理器缓存里
...
2. 然后B线程也执行了 temp = i + 1的操作，注意，此时A，B两个线程保存的 i 的值都是5，temp 的值都是6

3.  然后A线程执行了 i = temp （6）的操作，此时i的值会立即刷新到主存并通知其他线程保存的 i 值失效

4. 但是，此时并没有用来，因为，此时B线程已经将i值计算好了，线程B切回cpu时候，直接回写内存了。
...
7. static final：只占据一段不能改变的存储空间，必须在定义的时候进行赋值，否则编译器将不予通过
...
即释放对moniter的所有权，释放过程即moniter的计数器-1，若减完之后计数器不是0，则代表这个锁是可重入的，当前线程继续持有这把锁的所有权，若计数器变为0，则代表当前线程不再拥有该moniter的所有权，即释放锁
...
1. synchronized是Java底层Jvm级别实现的；Lock+Condition是Java语言级别实现，具有可控性和扩展性
...
1. 我们解析一个Excel里多个sheet的数据时，可以考虑使用多线程，每个线程解析一个sheet里的数据，等到所有的sheet都解析完之后，程序需要统计解析总耗时
2. 应用程序的主线程希望在负责启动框架访问的线程已经启动所有服务框架后再启动



解决方案

1. join()：主程序等待所有线程执行完，最后汇总
2. CountDownLatch



**实现原理：内部为Sync继承自AQS的内部类，主要还是在AQS那一套东西上修改而来**
...
1. countdownLatch是减计数器，CyclicBarrier是加计数器
...
> 用于**控制同时访问特定资源的线程数量**，用于控制并发线程的数量，它通过协调各个线程，以保证合理的使用公共资源
>
> 

应用场景：

1. 流量控制(限流)，特别是公用资源有限的应用场景，例如：数据库连接

2. semaphore初始化10个令牌，11个线程同时去调用acquire，会有一个线程拿不到令牌而阻塞，等待其他线程释放令牌

3. semaphore没有锁冲入的概念，10个令牌，一个线程重复调用11次acquire会造成线程阻塞

4. semaphore初始化有1个令牌，1个线程调用一次acquire方法，然后调用两次release方法，之后另外一个线程调用acquire(2)方法，此线程能够获取到足够的令牌并继续运行吗?答案：能，原因是release方法会添加令牌，并不会以初始化的大小为准

5. semaphore初始化有2个令牌，一个线程调用1次release方法，然后一次性获取3个令牌，会获取到吗? 答案：能，原因是release会添加令牌，并不会以初始化的大小为准。Semaphore中release方法的调用并没有限制要在acquire后调用

   



实现原理：**底层还是自己实现的自己的Sync内部类继承AQS**



#### Phaser

> JDK7新增了一个同步辅助类，它可以实现CyclicBarrier和CountdownLatch类似的功能，而且它支持对任务的动态调整，并且支持分层结构来达到更高的吞吐量

Phaser又称：阶段器，用来解决多个线程分阶段共同完成任务的场景问题。和CyclicBarrier、countdownlatch类似，都是等待一组线程完成工作后再执行下一步，协调线程的工作

但是Phaser可以动态注册需要协调的线程

![image-20230325153918685](https://pic-typora-qc.oss-cn-chengdu.aliyuncs.com/juc_img/202303251539585.png)
...
>  Exchanger用于进行两个线程之间交换数据，它提供一个同步点，在这个同步点，两个线程可以交换彼此的数据
...
> 从JDK5开始，把工作单元与执行机制分离开来。
>
> 工作单元包括Runnable、Callable （Java线程就是工作单元）
>
> 执行机制由Excutor框架提供
...
1. 降低资源消耗（线程不断的创建，然后使用后立刻销毁）
2. 提高响应速度（无需自己去创建线程，而是从线程池中获取线程）
3. 提高线程的可管理性
...
> Future表示一个任务的生命周期，是一个可取消的异步运算，可以把它看作一个异步操作的结果的占位符。它在未来的某个时刻完成，并提供对其结果的访问
>
> JUC中，许多异步任务类都是继承自Future，其中最典型的就是FutureTask
>
> FutureTask的线程安全由CAS保证



Future接口和实现类FutureTask，代表异步计算的结果

1. FutureTask为Future提供了基础的实现，如果任务尚未完成，获取任务结果的线程将会阻塞
2. 任务完成后，不能被重启或取消
3. FutureTask常用来封装Callable、Runnable、也可以用来作为一个任务提交到线程池中去执行





FutureTask实现了RunnableFuture，RunnableFuture继承自Runnable、Future。即FutureTask实现了Runnable与Future接口，因此FutureTask可以交给Executor执行，也可以由调用线程之间执行(FutureTask.run)，也能用来得到Callable的计算结果
...
2. SqlSessionManager
   - 经典例子：mybatis的session管理，创建连接、获取连接都从ThreadLocal里面获取
...
2. 线程资源一致性
   ...
   - JDBC操作事务的时候，一致性体现
   - JDBC保证，只要是同一个线程的请求，都返回同一个连接，这个就是用ThreadLocal实现的
   
4. 资源的持有

   - 项目中：用ThreadLocal\<String>存放用户信息

   - ~~~java
     ThreadLocal<String> t=ThreadLocal.withInitial(
     ()->{
         return "system";
     });
     ~~~
...
线程安全：当多个线程同时访问一个对象时，如果不考虑这些线程在运行时环境下的调度和交替执行，也不需要进行额外的同步，或者在调用方法进行任何其他的协调操作，调用这个对象的行为都可以获得正确的结果，那就称这个对象是线程安全的
...
1. 不可变
   - 不可变的对象一定是线程安全的
   - String类、Long、BigInteger等等类
2. 绝对线程安全
3. 相对线程安全
   - Vector
   - HashTable
   - synchronizedCollection()等等
4. 线程兼容
   - Vector对于ArrayList
   - HashTable对应HashMap等
5. 线程对立
...
是一种最常见也是最主要的并发正确性保障手段。同步是指在多个线程并发访问共享数据时，保证共享数据在同一个时刻只被一条（或者是一些，当使用信号量的时候）线程使用。而互斥是实现同步的一种手段，**临界区**（Critical Section）、互斥量（Mutex）和**信号量**（Semaphore）都是**常见的互斥实现方式**

Java中最基本的互斥同步手段就是synchronized关键字，其关键字经过Javac编译后，会在同步块的前后分别形成mointorenter和monitorexit两个指令



###### 为什么说Synchronized是重量级的？

Java的线程是映射到操作系统的原生内核线程之上的，如果要阻塞或唤醒一条线程，那么就需要操作系统来帮忙完成，这就不可避免陷入用户态到内核态的转换中，进行这种转换是需要耗费很多处理器时间。对于那些操作简单的同步块，状态切换消耗的时间往往比用户代码本身执行的时间还要长。



第二个就是JUC包中的相关东西：Lock接口等等



互斥同步面临的问题：进行线程阻塞和唤醒所带来的性能开销，这种同步也称为阻塞同步。互斥同步**属于一种悲观的并发策略**



##### 2.非阻塞同步

通俗地说，就是不管风险，先进行操作，如果没有其他线程争用共享数据，那操作就直接成功了；如果共享的数据的确被争用，产生了冲突，那再进行其他的补偿措施，最常用的补偿措施是不断地重试，直到出现没有竞争的共享数据为止。这种乐观并发策略的实现不再需要把线程阻塞挂起，因此这种同步操作被称为非阻塞同步，使用这种措施的代码也常被称为无锁（Lock-Free）编程。



通常，这就需要**硬件指令集的发展**来赖以支撑了。因为我们**必须要求操作和冲突检测这两个步骤具备原子性**。靠什么来保证原子性？如果这里再使用互斥同步来保证就完全失去意义了，所以我们只能靠硬件来实现这件事情，硬件保证某些从语义上看起来需要多次操作的行为可以只通过一条处理器指令就能完成
...
5. 加载链接/条件储存：Load-Linked/Store-Conditional

后两条是现代处理器新增的，这两条指令的目的和功能也类似。在Java里，最终暴露出来的是CAS操作
...
V 内存地址=>Java中可以理解为：某个变量的内存地址 

 A=>旧的预期值

B=>准备设置的新值

CAS（V，A，B）：当且仅当V符合A时，处理器才会用B更新V的值，否则不会执行更新。但是，不管是否更新了V值，都会返回V的旧值，这个过程是一个原子操作，执行期间不会被其他线程中断。即：CAS操作是一个原子操作



JDK5之后，Java类库才开始使用CAS操作，该操作由sun.misc.Unsafe类的compareAndSwapInt()等等几个方法包装来提供。HotSpot虚拟机在内部对这些方法做了特殊处理，即时编译出来的结果就是一条平台相关的处理器CAS指令，没有方法调用的过程，或者可以认为是无条件内联进去了



CAS存在的问题：ABA问题

解决：juc中提供了AtomicStampedReference，它可以通过**控制变量值的版本**来保证CAS的正确性，不过大部分ABA问题不会影响程序并发的正确性，这个类处于鸡肋位置，若需要解决ABA问题，改用传统的互斥同步比原子类更为高效



##### 3.无同步方案

要保证线程安全，也**并非一定要进行阻塞或非阻塞同步**，同步与线程安全两者没有必然的联系。
...
高效并发是JDK5->JDK6的重要改进！！HotSpot虚拟机开发团队花费了大量的资源去实现各种锁优化技术
...
锁消除的主要**判定依据来源于逃逸分析的数据支持**，如果判断到一段代码中，在堆上的所有数据都不会逃逸出去被其他线程访问到，那就可以把它们当作栈上数据对待，认为它们是线程私有的，同步加锁自然就无须再进行
...
代码所示连续的append()方法就属于这类情况。如果虚拟机探测到有这样一串零碎的操作都对同一个对象加锁，将会把加锁同步的范围扩展（粗化）到整个操作序列的外部，以代码清单为例，就是扩展到第一个append()操作之前直至最后一个append()操作之后，这样只需要加锁一次就可以了。
...
并不是用来代替传统重量级锁的，设计初衷是：**在没有多线程竞争的前提下，减少传统的重量级锁使用操作系统互斥量产生的性能消耗**

这就涉及到，HotSpot虚拟机中：对象的内存分布（尤其是对象头部分有关）

HotSpot虚拟机中的对象头分为两部分：

1. 存储对象自身的运行时数据：HashCode、GC分代年龄等等，官方称为MarkWord，这部分是实现轻量级锁和偏向锁的关键
2. 存储指向方法区对象类型数据的指针
...
它的意思是**这个锁会偏向于第一个获得它的线程**，如果在接下来的执行过程中，该锁一直没有被其他的线程获取，则持有偏向锁的线程将永远不需要再进行同步
...
4. 不保证原子性
   - i++、++i等不能保障原子性
   - 因为这些可以分解为：获取i、进行加操作、赋值（将i的值写回内存）三个步骤
5. 如何解决不保证原子性
   - sync
   - 原子类
