...
- 5.6之前，存储引擎会通过遍历索引定位基表中的行，然后返回给Server层，Server层再去为这些数据行进行WHERE后的条件的过滤
...
3. table:当前行正在访问的表
   ...
   - 当 from 子句中有子查询时，table列是 \<derivendN> 格式，表示当前查询依赖 id=N 的查询，于是先执行 id=N 的查询
...
## 并发事务处理带来的问题
 ...
 多个事务并发运行，有时会操作相同的数据来完成各自的任务（多个用户对同一数据进行操作），那么可能会导致以下问题
...
1. 读未提交-隔离级别：允许读取尚未提交的数据变更
   - 可能会导致脏读、幻读、不可重复读
2. 读已提交-隔离级别：允许读取并发事务已经提交的数据变更
   - Oracle默认隔离级别
   - 会出现幻读和不可重复读
3. 可重复读-隔离级别：同一字段的多次读取都是一样的(mvcc实现的又叫快照读)
   - Mysql默认隔离级别
   - 可能出现幻读
4. 串行化-隔离级别：事务AB，mysql在同一时刻只允许单个事务执行，事务A在操作数据库时，事务B只能排队等待
...
Mysql解决并发事务的方式：锁和MVCC机制

### 锁机制

MySQL主要通过读写锁来实现并发控制

- 共享锁（S锁）：读锁，事务在读取记录的时候获取共享锁，允许多个事务同时获取读锁
- 排它锁（X锁）：写锁/独占锁，不允许多个事务同时获取

1. Mysql的InnoDB引擎根据锁粒度的不同分为：
   - 行级锁：锁定当前数据行，锁的粒度小、加锁慢，发生锁冲突概率小，并发度高。InnoDB支持行锁和事务、MyISAM这两者都不支持
     - ==mysql行级锁是加在索引上的==
     - 为了更改数据，数据库必须在更改的行上施加独占锁，insert、update、delete、select for update语句会隐式的采用行锁定（有索引的前提下）
   - 表级锁：锁的粒度大，加锁快，开销小，但是锁冲突的概率大，并发度低
     - update、delete如果没有加索引条件，那么就升级为表锁

无论是表级锁、行级锁都存在共享锁和排它锁两类



MySQL四种隔离级别的实现方式

> 基于锁+MVCC共同实现，下面是几种隔离级别的加锁情况

1. **读未提交加锁情况**：没加锁
2. **串行化加锁情况**
   - 读的时候加的共享锁，也就是其他事务可以并发读取，但是不能写。
   - 写的时候加的排它锁(写锁、独占锁、X锁)，其他事务不能并发读和写
3. **读已提交加锁情况**：他们俩的底层实现采用的是MVCC（多版本并发控制）方式进行实现
4. **可重复读加锁情况**：他们俩的底层实现采用的是MVCC（多版本并发控制）方式进行实现
...
每个事务读到的数据版本可能是不一样的，在同一个事务中，用户只能看到该事务创建Read View之前已经提交的修改和该事务当前本身做的修改
...
> 普通的不加锁的select操作就是快照读
>
> 快照读的前提是隔离级别不是串行级别，串行级别下的快照读会退化成当前读

快照，即记录的历史版本，每条记录都可能存在多个历史版本（多版本技术）

- 快照读的情况下，如果读取的记录正在进行update/delete操作，读取操作是不会因为因此去等待记录上X锁的释放，而是==去读取该行的一个快照==
...
MVCC是维持一个数据的多个版本，使得==快照读与写操作没有冲突==的一个**抽象概念**，这个概念需要具体功能实现就是快照读

**MVCC就是为了实现  快照读-写 冲突不加锁**，当前读实际上是一种加锁的操作，是悲观锁的实现



只有在事务隔离级别为RC(读已提交)、RR(可重复读)下，InnoDB才会使用非一致性锁定读，即快照读

- RC级别下：对于快照读，总是读取被锁定行的最新一份快照数据
- RR级别下：对于快照读，总是读取本事务开始时的数据快照版本



当前读：一致性锁定读

> select * from xxx   lock in share mode(共享锁)
>
> select ........for update
>
...
1. **并发 快照读-写时：可以做到快照读操作不阻塞写操作，写操作不阻塞读操作**；提高了数据库的并发读写的性能
...
> MVCC的实现依赖于：版本链、undo log、read view以及记录中的3个隐式字段来实现
...
1. db_trx_id：表示最后一次插入/修改该行的事务id

   - delete操作视为更新，只是在记录头Record header中的deleted_flag字段进行了标识
   - 通过事务id的大小判断事务的时间顺序

2. db_roll_prt：(pointer) 回滚指针

   - 指向当前行记录的上一个版本，即：指向该行的undo log
   - 通过这个指针将数据的多个版本连接在一起构成undo log版本链

3. db_row_id：隐含的自增ID(隐藏主键)

   - 如果数据表没有主键，且没有唯一非空索引时，Innodb会自动以db_row_id产生一个聚集索引
...
1. 保证事务进行`rollback`时的`原子性和一致性`，当事务进行回滚的时候可以用undo log的数据进行恢复
2. 用于MVCC快照读
   - 当读取记录时，若该记录被其他事务占用或当前版本对该事务不可以，则通过读取undo log的历史版本数据，可以实现不同事务版本号都拥有自己独立的快照数据版本
...
1. insert undo log
   - insert操作产生的undo log
   - insert操作的记录只对当前事务本身可见，故该undo log可以在事务提交后直接删除，不用purge操作
   - 插入一条记录时，至少要把这条记录的主键值记下来，之后回滚的时候只需要把这个主键值对应的记录删掉就好了
2. update undo log（主要）
   - update、delete产生的undo log
   - 该undo log可能需要提供MVCC机制使用，所有事务提交时不能删除，而是放入undo log链表。在快照读或事务回滚不涉及该日志时，对应的日志才被purge线程统一清除
...
- 先对该行加排他锁，
- 然后把该行拷贝到undo log中，作为旧记录
- 拷贝完成后修改name为Tom，同时修改隐藏字段事务id，假设默认从1开始，之后递增，回滚指针指向拷贝到undo log的副本记录，即：上一个版本就是它
- 事务提交后释放锁

![image-20220327222930982](https://pic-typora-qc.oss-cn-chengdu.aliyuncs.com/mysql_img/202203272229183.png)

之后==事务2==也对该记录做了修改，将age改为30，同事务1进行同样的操作

![image-20220327223050366](https://pic-typora-qc.oss-cn-chengdu.aliyuncs.com/mysql_img/202203272230508.png)																																															 

可以看出，==不同事务或者相同事务的对同一记录的修改，会导致该记录的undo log成为一条记录版本线性表，既链表==，undo log 的链首就是最新的旧记录，链尾就是最早的旧记录
...
Read View 是 InnoDB 在执行一致性读操作时创建的一个快照，用于确保读操作看到的是事务开始时的数据状态，而不是其他事务提交后的最新数据。Read View 记录了当前活跃事务的列表，并使用这些信息来决定哪些数据版本对当前事务可见

read view的作用：主要用来做**版本可见性判断**
...
1. m_ids
   - 创建当前Read View时，系统中所有活跃事务的ID列表
   - 不包括当前事务自己和已经提交的事务
2. low_limit_id：
   - 目前出现过的最大的事务ID+1
   - 大于等于这个ID的数据版本不可见
3. up_limit_id
   - 活跃事务列表m_ids中，最小的事务id
   - 小于这个Id的数据版本均可见
4. creator_trx_id
   - 创建该Read View的事务Id
...
InnoDB引擎中，创建一个新事务后，执行每个Select语句前，都会创建一个快照(Read View)

- 快照中保存了当前数据库系统中，正处于活跃的事务的Id(m_ids)
- 即：系统中当前不应该被本事务看到的其他事务id列表
- 当用户在这个事务中要读取某个记录行数据的时候
  - InnoDB会将该行的隐藏字段Db_trx_id与Read View中的一些变量及当前事务ID进行比较
  - 判断是否满足可见性条件
...
- 主要是：把最新记录中的DB_TRX_ID取出来，与系统中当前活跃的事务的id去对比(m_ids，read view维护)
- 如果DB_TRX_ID跟read view的某些属性比较，不符合可见性，那就通过DB_POLL_PTR回滚指针去取undo log中的
  - 在undo log再去根据上述逻辑比较
  - 直到找到满足特定条件的DB_TRX_ID，那么这个DB_TRX_ID所在的旧记录就是当前事务能看见的最新的老版本

![image-20220327224520607](https://pic-typora-qc.oss-cn-chengdu.aliyuncs.com/mysql_img/202203272245831.png)

db_trx_id(最新修改该行的事务id，不一定是当前事务id，因为其他事务可能并发修改了)

up_limit_id：活跃事务列表中，最小事务id

low_limit_id：活跃事务列表中，最大事务id

creator_trx_id：创建该read vied的事务id

1234

1. 如果记录的db_trx_id<up_limit_id || db_trx_id==creator_trx_id

   - 表明最新修改该行的事务，在当前事务创建快照之前就提交了，所以该行记录的值，对当前事务是可见的
   - 或者当前事务ID等于creator_trx_id，那么说明这个数据就是当前事务自己生成的，自己生成当然可见

2. 如果db_trx_id>= low_limit_id

   - 表明最新修改该行的事务，是在当前事务创建快照之后才修改该行，所以该记录行的值对当前事务不可见，跳到步骤5
   - (类似于第二次select 前中间有其他事务进行了update，此时，看的还是最初的read view)

3. m_ids为空

   - 表明在当前事务创建快照之前，修改该行的事务都已经提交了，所以该记录行的值对当前事务可见的

4. 如果up_limit_id <=db_trx_id<low_limit_id

   - 表明最新修改该行的事务，在当前事务创建快照的时候，可能处于“活动状态”或“已提交状态”
   - 所以要对活跃事务列表m_ids进行查找
     - 如果m_ids中能找到db_trx_id
       1. 表明当前事务创建快照前，该记录的值被事务id为db_trx_id的事务修改了，但是没提交
       2. 当前事务创建快照后，该记录的值被id为db_trx_id的事务修改了
       3. 这些情况下，这个记录行的值对当前事务都是不可见的。跳到步骤5
     - 如果m_ids中没找到db_trx_id
       1. 表明id为db_trx_id的事务，在修改该记录行的值后，在当前事务创建快照前已经提交了
       2. 所以记录行对当前事务可见

5. 在该记录行的db_roll_ptr指针所指向的undo log取出快照记录，用快照记录的db_trx_id跳到步骤1，重新开始判断，直到找到满足的快照版本或返回空
...
https://javaguide.cn/database/mysql/innodb-implementation-of-mvcc.html#rc-%E5%92%8C-rr-%E9%9A%94%E7%A6%BB%E7%BA%A7%E5%88%AB%E4%B8%8B-mvcc-%E7%9A%84%E5%B7%AE%E5%BC%82

### MVCC是如何解决不可重复读的？

在RR隔离级别下，快照读可以在不加锁的情况下，解决不可重复的读和幻读
...
InnoDB在RR级别下，通过MVCC+Next key Lock解决幻读问题

1. 快照读：通过MVCC控制，不用加锁
   - RR隔离级别只会在事务开启后的第一次查询生成Read View，并使用至事务提交
   - 所以生成Read View后，其他事务做的更新、插入记录版本，对当前事务并不可见，实现了可重复读和防止快照读下的幻读
...
- 表级别的锁是MySQL各种存储引擎中最大颗粒度的锁机制（全局锁除外）

- 由于表级锁一次会将整个表锁定，所有可以很好的避免死锁问题
...
- 页锁的锁定颗粒的介于表锁和行锁之间，能提供的并发处理能力页锁介于二者之间。
...
缺点：由于资源锁定颗粒的最小，所有每次获取锁和释放锁需要做的事情更多，带来的消耗也就更大，此外行**级锁最容易发生死锁**
...
共享锁，也叫读锁，其他用户可以并发读取数据，但是在读锁未释放前，也就是查询事务结束前，任何事务都不能对数据进行修改(获取数据上的写锁)，直到已释放所有读锁

由于MVCC的存在，一般的select语句，InnoDB不会加任何锁，可以显示的给select共享锁或排他锁

~~~sql
#select 显示加共享锁写法：

select xxxxx      Lock in share mode; #mysql5.7开始
select * from user where name='qc' for share; #mysql8.0开始
~~~
...
> InnoDB维护的，用户无法手动操作意向锁，在为数据加共享/排他锁之前，InnoDB会先获取该数据行所在数据表的意向共享锁/意向排他锁
>
> 其设计主要目的是为了表明某个事务正在锁定一行或将要锁定一行
...
单个行记录上的锁，也就是日常认为的行锁。==主键和唯一索引都是行记录锁模式==，**InnoDB的行锁是加在索引上的**
...
锁定一个行记录的范围，但不包括记录本身（只不过它的锁粒度比记录锁的锁整行更大一些，他是锁住了某个范围内的多个行，包括根本不存在的数据）

Gap锁的目的

- 为了让其他事务无法在间隙中新增数据，即不允许在此范围内插入任务数据

- 为了==解决同一事务的多次读，出现幻读的情况==
...
- InnoDB默认的事务隔离级别RR已经解决幻读，就是临键锁（行锁+间隙锁）的功劳



当InnoDB扫描索引记录时，会先对选中的索引记录加上记录锁（Record Lock），再对索引记录两边的间隙上加上间隙锁

假设数据表中有记录id=1, 3, 5, 7，现在记录where id=5上加next-key lock，则会锁定区间(3, 5]，任何试图插入到这个区间的记录都会阻塞。

- ==左开右闭==
- 其效果相当于(3, 5)上的gap lock加5上的record lock
  - 而且gap lock是可重入的，相互不阻塞的，当其它事务试图获取(3, 5)的gap lock时，不会被阻塞
  - 但如果要获取5上的record lock，就会阻塞；如果要获取5上的next-key lock，同样会阻塞
...
了解即可。自增锁是一种特殊的**表级锁**，主要用于事务中插入自增字段，也就是我们最常用的自增主键id。
...
> 将数据库中的数据，分散存储到不同的数据库上，可用垂直分库、水平分库

1. 垂直分库：将单一数据库按照业务划分，不同的业务使用不同的数据库
   - 用户表、订单表、商品表：分别拆分到用户数据库、订单数据库、商品数据库
2. 水平分库：把同一张表按照一定的拆分规则，拆分到不同的数据库中，每个库可以位于不同的服务器上，这样就实现了水平扩展
   - 订单表：按照id将订单表切分为2张订单表，分别存储在不同的数据库中
...
1. 垂直分表：对数据表的列，按照一定的关系，进行拆分为多张表
2. 水平分表：把一张行数比较多的表，水平拆分为多张表
   - 水平拆分只能解决单表数据量大的问题，为了提升性能，通常会选择将拆分后的多张表，放在不同的数据库
   - 也就是说：==水平分表通常和水平分库同时出现==
...
分片算法：主要解决**==数据被水平分片后==**，数据究竟该存放在哪个表的问题

1. hash分片
2. 范围分片：按照特定的范围区间来分配数据
3. 映射表分片
   - 使用一个单独的映射表来存储分片键和分片位置的对应关系
   - 可以灵活的调整分片规则，可以自己配置需要的分片算法
4. 一致性哈希分片
   - 将hash空间组织成一个环形结构，然后按照顺时针规则确定数据分配哪个节点上
5. 地理位置分片
6. 融合算法分片：使用多种分片算法
7. .........
...
分片键：Sharding key，是数据分片的关键字段，它关系着数据的分布和查询效率

一般分片键应具备以下特点，并且，分片键可以是多个字段的组合

1. 具有共性：即能覆盖绝大多数的查询场景，尽量减少单次查询所涉及的分片数量，降低数据库压力

2. 具有离散性：即能将数据均匀的分散在各个分片上，避免数据倾斜

3. 具有稳定性：即分片键的值不会发生变化，避免数据迁移和一致性问题

4. 具有扩展性：即能支持分片的动态增加和减少，避免数据重新分片的开销
...
1. join操作
   - 同一个数据库中的表，被sharding在了不同的数据库中，导致无法join：需要手动进行数据的封装
   - 大厂DBA都建议尽量不用join，采用多次查询，业务层进行组装
     - 这种方法需要考虑业务上多次查询的事务性的容忍度
2. 事务问题
   - 单个操作涉及到多个数据库，那么数据库自带的事务特性就无法满足要求了
   - 分布式事务：Seate、等等
3. 分布式ID
   - 数据分布在不同数据库上，数据库的自增id就无法满足主键唯一了
   - 分布式全局唯一Id：雪花算法等
4. 跨库聚合查询问题
   - 分库分表后会导致常规的聚合查询操作变得很复杂（group by、order by等)
   - 需要在业务层面编写代码控制、使用中间件来协调分片间的通信和数据传输
5. ...............
6. 另外，分库分表后，需要DBA的介入，同时还需要更多的数据库服务器，这些都是考虑的成本
...
1. ShardingSphere：Apache开源
   - 包括Sharding-JDBC、Sharding-Proxy、Sharding-Sidecar
...
不过，现在很多公式对于数据量大的业务直接采用了：分布式数据库了

- 不在需要我们手动分库分表了（数据库层面已经做好了）
- 若公司条件允许，直接推荐这种方式
- 比如：**GreenPlum**(业界最快最高性价比的关系型分布式数据库、PostgreSql基础上实现的)、Googel的Spanner、阿里OceanBase、腾讯TDSQL等等
...
客户端实现分页功能的时候，要显示当前页的数据、当前所在页数、临近页面的按钮以及总页数等等。

这些数据随着翻页的进行能够动态的变化，为了实现这样的效果，一般会采取两种办法：**真分页**和**假分页**
...
**适用于数据量比较大的场景，数据不适合全量查出的情况**

~~~mysql
select * from user where id>10 limit 2,2
~~~
...
~~~mysql
#取第10w后面的100条
select * from user order by id limit 1000000,100
~~~

1. 范围查询

   - 如果保证Id连续性的前提下，根据Id范围进行分页是比较好的解决方案

   - ~~~mysql
     select * from user where id>100000 and id <100100
     select * from user where id>100000 limit 100
     ~~~

   - 局限性

     - id连续性要求高，通常难以保证
     - 排序需要按照其他字段而非id，那么也不适用了

2. 子查询

   - 先查询出limit第一个参数对应的主键值，再根据这个主键值去过滤并limit
   ...
   - 局限性
     ...
     - 子查询会产生新表，会影响性能，应避免大量使用子查询
     - id局限性，通常业务场景中，筛选到符合条件的id，这些id都是离散且不连续的

3. ==延迟关联==

   - 通过将limit操作转移到主键索引上，减少回表次数

   - ~~~mysql
     select u1.* from user
     inner join 
     (select id from user limit 1000000,100) u2
     on u1.id=u2.id
     ~~~

   - 原理

     - 子查询利用主键索引快速定位到目标分页的100条记录id
     - 通过join将子查询结果与主表关联，获得完整记录
...
4. 覆盖索引
   ...
   - ~~~mysql
     ...
     order by name
     limit 100000,100
     #如果只查询id,name,age  可以建立name和age的覆盖索引
     ~~~
...
https://javaguide.cn/database/mysql/mysql-questions-01.html#%E8%83%BD%E7%94%A8-mysql-%E7%9B%B4%E6%8E%A5%E5%AD%98%E5%82%A8%E6%96%87%E4%BB%B6-%E6%AF%94%E5%A6%82%E5%9B%BE%E7%89%87-%E5%90%97



阿里巴巴手册：单表超过500w或者容量超过2G，才推荐分库分表

- 但是，这这是一个大概的数字
- 实际情况是：每张表由于字段不同、字段所占的空间不同等，它们在最佳性能下可用存放的数据量也就不同

如何计算出每张表适合的数据量？

https://juejin.cn/post/7165689453124517896（牛逼）



## 一颗高度为3的B+Tree可以存放多少数据

B+Tree聚簇索引索引**节点**的单位是数据页，MySQL中B+树的一个节点大小为1页，即默认16k，可以修改，最大为64k，最小为4k



问题针对：存储引擎为Innodb的主键索引才会出现这个问题，因为Innodb数据文件和索引文件(主键、聚簇索引)是放在一起的

1. InnoDB中，我们常说的节点，被称为：页，Mysql页默认大小为16k
2. 页是InnoDB管理数据库的最小磁盘单位
3. 这16k的空间里，存储了页格式信息、行格式信息
   - 行信息又包含一些元数据和用户数据
   - 这些加进去后，大概每页还剩于15232即：15k左右
4. b+tree为叶子节点存放完整的数据，假设一条数据大小为1K，则叶子节点一页可以存放16条数据，B+树的叶节点就是数据页
5. 非叶子节点存放主键和指针，所以要看主键是什么类型的，假如为Integer，则长度为8字节，指针大小在innodb是6个字节，一个5字节的行标头，所以非叶子节点每页可以存15232/19=800个主键数据和指针
   1. B+Tree中，当一个节点索引记录为N条时，它就会有N个子节点
   2. 则三层B+Tree可以存800* 800个叶子节点




mysql遵循最大行大小长度略小于数据库页大小的一般

- 最少存放记录数：每页大小默认16k算，那么每条算最大可以为8k

  - 那么每页只能存放2条，2x800x800=128w..........
  - 这个数据量没想到吧哈哈哈哈哈
...
- 较多的存放记录数
  ...
  - 那么每个叶子节点可存放：16k/30B=500条

  - 那么理论上这个表可用存放500x800x800=3.2亿
...
- 常规的存放记录数
  ...
  - 简单起见，加上每条占用1k吧
  - 那么每个叶子节点可用存放：16k/1k=16条
  - 那么总共存放：800x800x16=1000w左右
...
通常情况下是MySQL死锁是相互等待对方资源释放资源，最后形成环路造成的
...
1. 设置超时时间
   ...
   - InnoDB默认50s等待锁超时，InnoDB会选择回滚undo  log量最小的事务，从而打破死锁
   
2. wait-for graph
   - 此死锁检测算法是==InnoDB使用的死锁解决办法==
