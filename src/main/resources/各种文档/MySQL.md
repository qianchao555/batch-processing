...
7. key_len
   ...
   - 例如：单列索引时候    int  not null /  null   该列的值为4或5  null的时候要加1
   - 另外varchar边长字段 还会在上面的基础上再+2
   - 但是，key_len只代表where中用于过滤时被选中的索引列，并不包含order by、group by这两部分被选中的索引列
...
10. Extra：额外信息
   ...
   - Using index：这发生在对表的请求列都是同一索引的部分的时候，返回的列数据只使用了索引中的信息，而没有再去访问表中的行记录。是性能高的表现，即：**出现索引覆盖**
