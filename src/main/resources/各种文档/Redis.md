...
| SetEx key seconds value  | 组合命令，设置key的值并设置过期时间 |
...
前面分布式锁都是单机版本的。Redis一般都是集群部署的，集群下采取RedLock+Redisson来解决上述问题
...
> 布隆过滤器可以检索一个元素是否在一个集合中。它的优点是空间效率和查询时间都比一般的算法要好的多，缺点是有一定的误识别率和删除困难



Redis4.0版本中提供的新功能，它被作为插件加载到Redis服务器中，**给Redis提供强大的去查功能**

相比于Set集合的去查功能而言，布隆过滤器在空间上能节约90%以上，但是它的不足之处在于**去重率大约在99%左右，即存在1%左右的误判率。**这种误差是由布隆过滤器的自身结构决定的，**但是这种误差在处理海量数据时几乎可以忽略**

应用场景：

1. 海量数据去重
2. 公司利用bloom filter去重kafka数据
2. 应用在一些需要快速判断某个元素是否属于集合，但是并不严格要求100%正确的场合
2. 对网络爬虫进行过滤，爬过的url不再爬取
2. 邮件过滤，使用布隆过滤器来做黑名单过滤
2. 解决Redis缓存穿透问题（明天看看）！！！
...
[在线布隆过滤器在线计算网址](https://krisives.github.io/bloom-calculator/)

使用Bloom Filter有三个重要的值，可以通过这个网址得到我们想要的这几个参数大小

1. p：错误率：falseProbability
2. k：哈希函数的个数，当然hash函数与比较重要
3. m：bloom filter位数组的大小
4. n：插入字符串的个数

理论上来讲：m/n 越大，错误率越低，但消耗的空间会越多

例如：4亿级别的数据错误率在1/亿情况下，需要的bitset位大小、hash函数个数

![image-20230309221956201](https://pic-typora-qc.oss-cn-chengdu.aliyuncs.com/redis_img/202303092219567.png)
...
// 预估元素个数：expectedInsertions = 55000000
// 错误率：falseProbability = 0.03
...
即：redis集群时，出现的问题，可通过Redisson的RedLock或Zookeeper分布式锁解决
...
> 有些时候，Redis实例需要装载大量用户在短时间内产生的数据，数以百万的keys需要被快速的创建



方式一：使用Luke协议，通过redis-cli -pipe发送数据到服务器

使用正常模式的Redis 客户端执行大量数据插入不是一个好主意：因为一个个的插入会有大量的时间浪费在每一个命令往返时间上。Redis2.6开始，redis-cli**支持一种pipe mode的管道模式用于执行大数据量的插入工作**

~~~shell
cat data.txt | redis-cli --pipe

例如：data.txt存放有大量的例如set key v1   set key v2 .....这样的命令
~~~
...
1. 接口注解，利用key的特性，防止重复提交，实现幂等性
2. 分布式锁
3. 中间数据存储（存储需要下载的excel数据）
4. 存储配置信息（SLA配置，使用hash为每一个小功能做天数的配置）
...
不存在最佳模式，根据具体的业务场景选择合适的缓存读写模式



1. Cach Aside Pattern：旁路缓存模式
2. Read/Write Through Pattern：读写穿透
3. Write Behind Pattern 异步缓存写入



Cash Aside Pattern旁路缓存模式

> 使用较多的一个缓存读写模式，比较适合读请求较多的场景
...
> 1. 很简单，很多时候，在复杂一点的缓存场景中，缓存的值不单单是从数据库中直接取出来，而是可能通过联表、复杂的运算后，才计算出来的新的缓存值
> 2. 更新缓存的代价，有时候是很高的



==问题2==：写数据过程中，可以先删除cache在更新db么？

> 肯定是不行的，可能造成数据库和缓存数据不一致问题
>
> 请求1先把cache的A数据删除—>请求2随后读取数据A的话从数据库读取—>请求1再把A数据更新到cache
>
> 此时，请求2读取到的A值与cache中的值不一样



==问题3==：写数据过程中，先更新db，再删除cache就没有问题了吗？

> 理论上来说，可能会出现不一致问题，不过概率非常小，因为缓存的写入速度远大于数据库的写入速度
>
> 例如：cache没有数据A的前提下
>
> 请求1从db读取老的数据A—>请求2更新db中数据新A（此时cache没有，不用删除）—>请求1将数据A的旧值写入cache
>
> 此时，造成数据库与缓存不一致情况
...
1. 首次请求，数据一定不在cache的问题
   - 将热点数据提取放入cache中来解决
2. 写操作频繁的话，导致cache中的数据频繁被删除，会影响缓存命中率
   - 数据库与缓存强一致场景：更新db的同时，更新cache，不过需要加一个锁/分布式锁，来保证更新cache时不存在线程安全问题（串行化）
   - 可短暂允许数据不一致场景：更新db的时候，通用更新cache，但是给缓存加一个较短的过期时间，这样就可以保证即使数据不一致的话影响比较小



Read/Write Through Patter读写穿透

> 这种模式是 服务端把cache视为主要数据存储
>
> 这种模式非常少见，因为我们使用较多的Redis并没有提供将数据写入db的功能

写

1. 先查cache，cache不存在，直接更新db
2. cache存在，则更新cache，然后cache服务自己更新db(同步更新db)

读

1. 从cache读取数据，读取到就返回
2. 没有读取到，先从db加载，然后写入到cache后，然后返回响应
...
> 由cache服务 来负责cache和db的读写
>
> 与Read/write through相似，只不过一个是同步更新cache和db，一个是只更新缓存，不直接更新db，而是改为异步批量更新db
...
1. 所有O(n)时间复杂度度的命令
   ...
   - ......
2. save创建rdb快照
   - save：手动同步保存，会阻塞redis主线程
   - bgsave：自动保存，fork子进程时会阻塞，不会阻塞redis主线程，默认选项
3. aof
4. 大key
   - 如果一个key对应的value所占用的内存很多，那么这个key可以看作大key
   - 一个不是特别精确的参考标准
     - string类型的value超过1MB
     - 复合类型的value包含的元素超过5k个
   - 客户端阻塞：redis执行命令是单线程的，操作大key比较耗时，从客户端来看就是很久没有响应
   - 引发网络阻塞：若一个key大小是1M，每秒访问量1000，那么每秒会产生1000MB的流量，对于普通千兆网卡的服务器来说是灾难性的
   - 阻塞工作线程：如果del删除大key时，会阻塞工作线程，这样就没法处理其他命令
5. 查询大key：如何避免性能问题
   - 最好选择在从节点上操作
   - 通过分析rdb文件查找大key，前提是采用了rdb持久化
6. 删除大key
   - 释放内存只是第一步，在应用释放内存时，操作系统会把释放的内存插入一块空闲内存块的链表，以便后续管理和分配，这个过程需要一定的时候，而且会阻塞当前释放内存的应用程序
   - 所以，一下子释放大量内存，会造成Redis主线程的阻塞，从而其他所有请求可能会超时，超时越来越多，会造成reids连接耗尽，产生各种异常
   - 建议采用分批次、异步删除的方式进行
7. 清空数据库
   - 和删除大key类似的道理
8. 集群扩容
   - Redis集群扩容可以进行节点的动态扩容、缩容，半自动的目前是
   - 扩缩容的时候，需要进行数据迁移。redis为了保证迁移的一致性，迁移所有操作都是同步操作
9. Swap内存交互
   - 类似windows的虚拟内存，当内存不足时，将硬盘空间虚拟成内存使用
   - Swap对于Redis来说是致命的，因为内存与硬盘的读写速度相差几个量级，会导致Redis性能急剧下降
