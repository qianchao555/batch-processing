...
MongoDB提供了2种API来使用事务

1. 使用与关系型数据库类似的预防：start_transaction、commit_transaction，称为核心API
2. 称为回调API，这也是事务的推荐用法
...
许多应用程序需要进行==某一类的数据分析==。MongoDB为使用聚合框架原生地进行分析提供了强大的支持
...
聚合框架基于管道的概念，使用聚合管道，可以从Mongo集合获取输入，并将该集合中的文档传递到一个或多个阶段，每个阶段可对其输入执行不同的操作



聚合管道中，一个阶段就是一个数据处理单元。它一次接收一个输入文档流，一次处理一个文档，并且一次产生一个输出文档流

每个阶段都会提供一组旋钮或可调参数，通过控制它们来设置该阶段的参数，以执行任何感兴趣的任务
...
==模式设计，即：在文档中表示数据的方式==，最好的方式是按照应用程序看到的形式来表示我们的数据

因此，在为模式进行建模之前，首先需要了解查询和数据访问的方式



下列是在设计模式时需要考虑的几个关键方面

1. 限制条件
   - 一些数据库或硬件的限制需要了解
   - mongo文档大小、从磁盘读写完整文档、更新会重新整个文档、文档级别进行原子更新等
2. 查询和写入的访问模式
   - 需要确定并量化应用程序和系统最大的工作负载
   - 比如：并不常使用的数据放在不同的集合中
3. 关系类型
   - 根据应用程序的需要，以及文档之间的关系，来考虑哪些数据是相关的。设计时：考虑是嵌入到文档、引用文档
   - 需要弄清楚：如何在不执行其他查询的情况下引用文档，以及当关系发生变化时，需要更新多少文档
   - 还需要考虑：数据结构是否易于查询
4. 基数
   - 确定好文档和数据的关联方式后，应该考虑这些关系的基数
   - 例如：一对一、一对多、多对多、一对百万、多对百万等
   - 
...
>
> 虽然，默认情况下为应用程序定义模式是非必需的，但这是一个很好的实践！！
...
> 当文档拥有大量数据，而其并不常用时，子集模式就非常适用

产品评论、文章评论、电影中的电影信息等都是这个模式的应用场景案例
...
> 需要大量的Join操作来聚集那些需要频繁访问的数据，这时，可以采用扩展引用模式
...
> 在所需要的计算非常又挑战性或消耗的资源昂贵时，如果精度不是首要考虑的因素，可以适用该模式
...
举例：存放一个user集合，应该如何设计？

- 用户首选项
  - 这些信息可能与用户同时查询出来，所以，通常采用内嵌方式
- 最近活动
  - 这个字段取决于增长变化频率
- 好友
  - 通常，好友信息不应该采用内嵌到用户文档中
- 所有由用户产生的内容
  - 这个字段不应该内嵌到用户文档中
...
一旦确定了文档结构，就需要将它们放入哪些集合或数据库中



通常来说，类似模式的文档，应该保存在一个集合中。MongoDB通常不允许合并来自多个集合的数据

MongoDB4.2开始，可以使用$merge运算符，在聚合操作时，将来自一个数据库的聚合结果存储到另一个数据库以及该库的另一个集合中
...
> MongoDB中，通过查询聚合语句，分析定位慢查询/聚合分析
