...
Apache Kafka 的一个关键依赖是 Apache Zookeeper，它是一个分布式配置和同步服务。Zookeeper 是 Kafka 代理和消费者之间的协调接口。Kafka 服务器通过 Zookeeper 集群共享信息。Kafka 在 Zookeeper 中存储基本元数据，例如关于主题，代理，消费者偏移(队列读取器)等的信息，控制器选举等操作的分布式协调器
...
1. broker在zk中保存为一个临时节点，节点路径为：/brokers/ids/[brokerid]，每个节点会保存对应broker的ip以及端口等信息
...
job处理、上下游传递数据、日志发送kafka然后采用ELK那一套来对日志进行处理
