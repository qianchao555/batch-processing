...
> 面向分布式微服务架构的轻量级高可用**流量控制组件**。

主要==以流量为切入点==，从流量控制、熔断降级、系统负载保护等多个维度保护服务的稳定性
...
互联网应用中，有很多突发性的高并发访问场景，在没有任何保护的机制下，所以的流量进入服务器，很可能导致整个系统不可用。

为了保证系统在这些场景中仍然能稳定运行，就需要采取一定的系统保护策略，常见的保护策略：服务降级、限流、熔断等等
...
- 通过限制并发访问数或者限制一个时间窗口内允许处理的请求数量来保护系统，一旦达到限制数量则对当前请求进行处理，采用对应的拒绝策略。
...
然而，在第一个周期的最后5秒和下一个周期的开始5秒时间段内，分别涌入100的访问量，虽然没有超过每个周期的限制量，但是整体上10秒内已达到200的访问量，已远远超过服务器的负载能力，由此可见，计数器算法方式限流对于周期比较长的限流，存在很大的弊端
...
1. Sentinel核心库(Java客户端)：不依赖任何框架或库，能够运行与Java8及以上的版本的运行时环境，同时对Spring Cloud、Dubbo等微服务框架提供了很好的支持
2. Sentinel控制台(Dashboard)：Sentinel提供了一个轻量级的开源控制台，主要负责管理推送规则、监控、管理机器信息等功能。基于SpringBoot开发，打包后可直接运行，不需要额外Tomcat等应用容器
...
1. 资源
   ...
   - 只要是通过Sentinel API定义的代码，就是资源，就能被Sentinel保护起来
2. 规则
   - 围绕资源的实时状态设定的规则，可以包括流量控制、熔断降级规则、系统保护规则等
   - 所有规则可以动态实时调整
...
- 查看机器列表及健康状态
- 监控（单机和集群聚合）
  - 簇点链路（单机调用）：该页面实时的拉取指定客户都资源的运行情况
  - 实时监控（汇总资源信息 集群聚合）：仅存储5分钟以内的数据，若要持久化，需要通过实时监控接口来定制
- 规则管理和推送
  - doshboard的推送规则：原始模式（http api)、pull模式、push模式
  - 规则管理：doshboard上面默认就是使用的原始模式管理
- 鉴权
...
Seata是一个分布式事务处理框架，阿里巴巴和蚂蚁金服共同开源的分布式事务解决方案

Seata为用户提供了AT、TCC、SAGA、XA事务模式，为用户打造一站式分布式解决方案
...
- RM（Resource Manager)：资源管理器
  - 它是资源的管理者（可以理解为各服务使用的数据库）
  - 它负责管理分支事务上的资源，向TC注册分支事务，汇报分支事务状态，驱动分支事务的提交或回滚
...
三个组件相互协作，**TC 以 Seata 服务器（Server）形式独立部署，TM 和 RM 则是以 Seata Client 的形式集成在微服务中运行**，其整体工作流程如下图。
...
Automatic(Branch) Transaction Mode：是依赖于支持本地ACID事务的关系型数据库的

Seata提供了AT、TCC、Saga、XA四种事务模式，可以快速有效地对分布式事务进行控制，四种模式中，使用最多、最方便的就是AT模式。

- AT模式可以应对大多数的业务场景，且基本可以做到无业务入侵，开发人员有更多的精力关注于业务逻辑开发
- 不过具体使用哪种模式，需要根据自身业务而定的，没有银弹
...
1. 解析sql，得到sql的相关信息
2. 查询前镜像：根据解析到的条件信息，生成查询语句，定位数据
3. 得到前镜像后：执行业务SQL，更新记录
4. 查询后镜像：根据前镜像的结果，通过主键定位数据
5. 插入回滚日志：把前后镜像数据、业务SQL相关信息组成一条日志回滚记录，插入到undo-log表
6. 提交前，向TC注册分支
7. 本地事务提交：业务数据的更新和前面步骤生成的undo-log表一并提交
8. 将本地事务提交结果上报TC



![image-20220420220151103](https://pic-typora-qc.oss-cn-chengdu.aliyuncs.com/springcloud_img/202204202202173.png)
...
1. TC接收到所有事务分支的事务状态汇报之后，决定对全局事务进行**提交**
   - 把请求都放入一个异步队列中，马上返回提交成功的结果给TC
   - 异步任务阶段的分支提交请求，将异步和批量的删除相应的undo-log表记录
...
3. 数据校验：拿undo-log中的后镜像与当前数据进行比较
   - 如有不同，说明数据被当前全局事务之外的动作做了修改，那么根据配置的策略来处理
4. 根据undo-log中前镜和业务SQL的相关信息生成并执行回滚语句
5. 提交本地事务。并把本地事务的执行结果上报TC
...
所谓的隔离性是指多个用户并发访问数据库时，数据库为每个用户开启的事务不能被其他事务的操作所干扰，多个并发事务之间要相互隔离

AT模式中，多个全局事务操作业务表时，它的事务隔离级别是基于全局锁来实现的

全局锁：分支事务准备对业务表修改时，需要先向TC请求获取全局锁
...
1. 在第一阶段本地事务提交之前，确保拿到该记录的全局锁
2. 如果拿不到全局锁，则不提交本地事务，需要去等待获取全局锁
3. 获取全局锁的尝试会在一个范围内限制，如果超出时间范围将会放弃全局锁的获取，并且回滚本地事务，释放本地锁
...
在数据库本地事务隔离级别为Read Committed或者以上时，Seata AT事务模式的默认全局事务隔离级别是Read Uncommitted

- 在该隔离级别，所有事务都可以看到其他未提交事务的执行结果，产生脏读。

- 这在最终一致性事务模型中是允许存在的，并且在大部分分布式事务场景中都可以接受脏读

在某些特定场景中要求事务隔离级别必须为Read Committed，目前Seata是通过SelectForUpdateExecutor执行器对SELECT FOR UPDATE语句进行代理的

==SELECT FOR UPDATE语句在执行时会申请全局锁==

普通的select语句是可能读取到其他事务“未提交”的数据的

如图所示，如果全局锁已经被其他分支事务持有，则释放本地锁(回滚SELECT FORUPDATE语句的本地执行）并重试。在这个过程中，查询请求会被“BLOCKING”，直到全局锁被拿到，也就是读取的相关数据是“已提交”时才返回
...
> 注意：AT模式需要在自己的业务系统的数据库中，创建undo-log表。这个并不是数据库自带的undo log日志

undo-log表的作用：记录事务操作前的数据快照
...
Seata AT模式：是基于2PC改进而来的，数据最终一致性的分阶段事务模式

### Seata TCC模式

TCC模式和2PC很像

Seata TCC模式 则与Seata AT模式很像



Seata AT模式是基于：本地ACID事务的关系型数据库

1. 一阶段prepare：在本地事务中，提交业务数据的更新与相应的回滚记录undo-log
2. 二阶段
   - commit：马上成功结束，自动异步批量清理回滚日志
   - rollback：通过回滚日志，自动生成补偿操作，完成数据回滚

Seata TCC模式：不依赖底层数据资源的事务支持

1. 一阶段prepare：调用业务层自定义的prepare逻辑
2. 二阶段
   - commit：调用业务层自定义的commit逻辑
   - rollback：调用业务层自定义的rollback逻辑

==Seata的TCC模式，就是将自定义的分支事务，纳入到全局事务的管理中去==
...
==Seate Saga模式是Seata提供的长事务解决方案==，该模式中，业务流程中的每个参与者都提交本地事务，当出现某一个参与者失败，则补偿已经成功的参与者，一阶段正向服务和二阶段补偿服务都由业务层开发实现
...
1. 业务流程长、业务流程多
2. 参与者包含其他公司或遗留系统服务，无法提供TCC模式要求的三类接口
...
Seat Sage模式实现：基于状态机引擎

1. 通过状态图来定义服务调用的流程，并生成json状态语言定义文件
2. 状态图中，一个节点可以是调用一个服务，节点可以配置它的补偿节点
3. 状态图json由状态机引擎驱动执行，当出现异常时，状态引擎反向执行已成功节点对应的补偿节点，将事务回滚
   - 异常发生时，是否进行补充，也可以用户自定义
4. 可实现服务编排需求，支持单向选择、并发、子流程、参数转换、参数映射、服务执行状态判断、异常捕获等
...
2. 事件
   - 触发状态转换的外部或内部信号
   - 事件可以是用户输入、定时器触发、传感器信号等
3. 转换
   - 系统从一个状态切换到另一个状态的过程
   - 转换由事件触发，并可能伴随动作的执行
4. 动作
   - 在状态转换过程中执行的操作
   - 动作可以是函数调用、输出信号、状态更新等
5. 初始状态
6. 终止状态
...
Saga不是基于2PC的，而是通过补偿操作实现数据的最终一致性，而不是通过锁定资源和回滚操作



### Seata XA模式

seata xa模式是基于标准的2PC协议。它通过数据库的XA接口，实现分布式事务管理
...
同样的，Seata XA模式也是基于2PC完善的

==XA整体机制==

> 利用事务资源对XA协议支持，==以XA协议的机制来管理分支事务==的一种模式



1. 执行阶段
   - 可回滚：业务SQL操作放在XA分支中进行，由资源对XA协议的支持来保证回滚
   - 持久化：XA分支完成后，执行XA prepare，同样，由资源对XA协议的支持来保证持久化
2. 完成阶段
   - 分支提交：执行XA分支的commit
...
互联网应用，最关键的就是保证高可用。在此场景下，一些大佬在cap、base理论基础上提测了柔性事务。

==柔性事务就是：分布式理论的AP，遵循Base理论，允许一定时间内不同节点的数据不一致，但是要求最终数据一致性==
...
- **本地消息表、MQ事务、最大努力通知** 这些都属于柔性事务



### 刚性事务

刚性事务追求的目标：分布式理论的CP，对数据要求强一致性

刚性事务，首先要知道XA协议

- XA规范是：X/Open组织提出的分布式事务处理规范，分布式事务处理的工业标准
- ==XA协议是一个基于数据库的分布式事务协议==
- 分为两部分：事务管理器TM、本地资源管理器RM
  - 事务管理器作为一个全局的调度协调者，负责对各个RM统一号令提交或回滚

基于XA协议

- 2PC、3PC就是属于刚性事务，是基于XA协议衍生出来的
- Oracle、Mysql等数据库都实现了XA接口
- JTA、JTS：Java事务规范与实现
...
XA规范，是一种通用规范，其引入了几个组件：

1. AP：Application Program，应用程序本身

2. RM：Resource Manager，资源管理器

   - 也就是事务的参与者，大部分情况下就是指的：数据库

   - 一个分布式事务，往往涉及多个RM

3. TM：Transaction Manager，事务管理器

   - 负责管理全局事务，分配事务唯一标识，监控事务的执行进度，负责事务的提交、回滚、失败恢复等



Two-Phase Commit，两阶段提交

> 区分：数据库的二阶段提交redo log、binlog的提交时间段，这样是解决主从数据恢复不一致问题
>
> redo log的写入拆为2个阶段
>
> prepare阶段记录：redo log
>
> 事务提交后写记录：binlog 
>
> commit阶段再写：redo log
...
将事务的提交分为2个阶段：准备阶段、提交阶段



准备阶段Prepare

> 准备阶段的核心是：询问事务参与者执行本地数据库事务操作是否成功

1. 事务协调者/管理者：向所有涉及到的RM(资源管理者)发送消息“你是否可以执行事务操作”，并等待其回答
2. RM接收到消息后，开始执行本地数据库事务==预操作==（比如：写redo log、undo log)等，此时本地事务并不会提交
3. RM如果执行本地数据库事务预操作成功，那就给TM回复”Yes"，表示本地数据库准备就绪，否则回复“No"



提交阶段Commit

> 提交阶段的核心：询问事务参与者，本地事务是否成功

==当所有RM都是”就绪“状态==

1. TM向所有RM发送消息：你们可以commit提交本地事务了
2. RM收到消息后提交本地事务，执行完成之后，释放整个本地事务期间所占用的资源
3. RM回复：事务已经提交（ACK消息）
4. TM收到所有RM的ack消息后，整个分布式事务过程正式完成



==任一事务参与者是未就绪状态时==

1. TM向所有RM发送消息：你们执行回滚操作（Rollback消息)
2. RM收到Rollback消息后，执行本地事务回滚操作，执行完之后，释放整个事务期间占用的资源
3. RM回复：事务已经回滚了（Ack消息）
4. TM收到所有RM的ack消息后，中断分布式事务
...
- 准备阶段
  - 向RM发送准备请求，顺带询问一些信息（比如：事务参与者能否执行本地数据库事务操作）
  - RM回复：Yes、No、直接超时
  - 如果任意RM回复No或直接超时的话，就中断分布式事务（向所有RM发送’Abort"消息），否则进入预提交阶段
- 预提交阶段
  - TM向所有RM发送”预提交“请求，RM回复：Yes、No、直接超时
  - 所有RM都返回Yes，RM才会执行本地数据库事务预操作（比如：写redo log/undo log等）
  - 任意RM回复No或超时，中断分布式事务，向所有RM发送”Abort"消息。否则进入提交阶段
- 提交阶段
  - TM向所有RM发送“执行事务提交请求”，RM收到消息后，正式提交事务，并在事务完成后释放占用的资源
  - TM收到所有RM正确提交事务的消息，表示事务正常
  - 如果任意一个RM没有正确提交本地事务或超时，就中断分布式事务，TM向所有RM发送“Abort"消息
    - RM收到”Abort"后，执行本地数据库事务回滚
...
> 属于一种柔性事务解决方案，与2PC很像，不过2PC是应用于DB层面的，而TCC是应用于业务层
>
> 核心思想：==针对每个操作，都要注册一个与其对应的确认(try)和补偿(cancel)==
>
> Try、Confirm、Cancel三个阶段

1. Try阶段：尝试执行
2. Confirm确认阶段
   - 确认执行，所有RM的Try阶段执行成功，就会执行Confirm，否则执行Cancel
3. Cancel取消阶段
   - 取消执行，释放Try阶段预留的业务资源

一般而言，使用TCC模式的时候，需要我们自己代码实现try、comfirm、cancel这三个方法

TCC模式不需要依赖底层数据资源的事务支持，但是需要我们手动实现更多的代码，属于业务入侵型的一种分布式事务解决方案
...
> 其实是对本地消息表的封装，将本地消息表基于MQ内部，其他方面基本和本地消息表一致



### 最大努力通知

> 也称为定期校对，是对MQ事务的进一步优化



### Saga

Sage事务理论的论文提出，早于分布式事务概念的提出。是分布式事务领域最有名气的解决方案之一

==Sage属于长事务解决方案==，其核心思想是将长事务拆分为多个本地短事务，每个本地小事务处理完后，发送消息或事件来触发Saga中的下一个本地事务的执行。同时，每个子事务都有一个对应的补偿操作

==saga的设计目标是确保最终一致性，而不是严格的隔离性==

如果T1、T2~~Tn这些短事务都顺利完成，那么整个事务顺利完成，否则，将采用恢复模式

- 反向恢复
  - 如果Ti短事务提交失败，则补偿所有已经完成事务
  - 执行顺序T1,T2....Ti(失败)，那么Ci.....C2，C1(补偿)
- 正向恢复
  - 如果Ti短事务提交失败，则一直重试，直到成功为止，如何依次往下执行
  - 执行顺序：T1,T2....Ti(失败,一直重试)，Ti.....Ti+1.....Tn





Sage实现方式有多种，最流行的实现方式

1. 基于事件的方式
   - 这种方式没有协调中心，处于当前Saga下的各个服务，还产生某类事件，或监听其它服务产生的事件，并决定是否需要针对监听的事件做出响应
2. 基于命令的方式
   - 由一个协调中心来协调大家工作
   - 协调中心告诉Saga的参与方应该执行哪一个本地事务
...
1. Saga模式不保证事务的隔离性：可能会导致脏读、幻读、不可重复读等问题
...
- Seata：致力于微服务架构下提供高性能和简单易用的分布式事务服务。经历过双11的实战考验
- Hmily：高性能、零侵入，金融级分布式事务解决方案
- Raincat：基于2PC的分布式事务中间件
...
Nginx也是Web服务器，不过是轻量级的，占用内存少、启动快、高并发能力强，在互联网项目中广泛使用

Nginx特点

- 占用内存少，1w个非活跃的Http Keep-Alive连接，再Nginx中仅消耗3MB内存，这是Nginx支持高并发连接的基础
- 高并发：单机支持10w+并发连接
- 跨平台
- 扩展性好：第三方插件支持良好
- 安装简单、稳定性好、免费开源
...
一般所处的位置：用户-->nginx-->服务网关层-->具体服务

- 反向代理：代理目标服务器，暴露反向代理服务器的地址，隐藏真实服务器IP



与SpringCloud gateway的关联

1. gateway是前端到后端服务器之间的对内网关，nginx是用户到前端工程的对外网关
1. 外部请求首先经过Nginx层进行代理，代理分发到网关服务，网关服务再分发请求给具体的机器
...
一般是客户端直接向目标服务器发送请求并获取内容，使用正向代理后，客户端改为向代理服务器发送请求，并且指定目标服务器，然后由代理服务器和目标服务器进行通信，转交请求并获取内容，再返回给客户端

正向代理隐藏了真实的客户端，为客户端收发请求，使真实客户端对目标服务器不可见

正向代理例子：访问google

- 梯子会把访问Google的客户端请求，代理到一个可以直接访问Google网站的代理服务器上去
- 代理服务器会把Google的返回内容，再转发到客户端
...
如果一台服务器不能很好的处理客户端请求的话，比较简单的办法就是增加几台服务器，部署相同的服务集群来处理用户请求

Nginx将收到的客户端请求，按照一定的负载均衡策略，均匀地分配到服务器集群中所有的服务器上。这就叫Nginx的负载均衡

除此之外，Nginx还带有健康检查功能（服务器心跳检查），会定期轮询服务器集群中所有服务器，发送健康检查请求，来检查集群中是否有服务器异常
...
- IP哈希：基于客户端Ip地址进行hash

  - 同一个ip的请求总是被分配同一台服务器

- 最小连接数

  - 新请求出现时，遍历服务器节点列表，并选取其中活动连接数最小的一台服务器来响应当前请求
...
Nginx结合Keepalived+虚拟IP，来实现高可用

KeepAlived

- 一个由C编写的开源路由软件，是Linux下的一个轻量级的负载均衡和高可用解决方案
- KeepAlived的负载均衡依赖于：Linux虚拟服务器（IPVS即：IP Virtual Server，是内置在Linux内核中的传输层负载均衡器)内核模块，提供第四层负载均衡



Keepalived+Nginx实现高可用方法

1. 2台Nginx，一主，一备
2. 2台Nginx上安装并配置Keepalived
3. 为2台Nginx服务器绑定同一个虚拟IP
4. 编写Nginx检测脚本，用于通过Keepalived检测Nginx主服务器的状态是否正常

如果主服务器宕机，会自动进行故障转移，备用Nginx服务器升级为主服务器

- 这个切换是透明的，因为使用的虚拟IP，虚拟IP不会改变
...
CDN、负载均衡、数据库优化、SQL优化、代码优化

## CDN

Content Delivery Network，内容分发网络

- 内容：指的静态资源，图片、视频、文档、js、css、html等
- 分发网络：指的是将这些静态资源，分发到位于多个不同地理位置机房中的服务器上。这样，就可以实现静态资源的就近访问，例如北京的用户直接访问北京机房的数据

全站加速：不同云服务商叫法不同，腾讯云的ECDN、阿里的DCDN
...
- 通过==预热==的方式，将源站的资源同步到CDN节点中
  - 用户首次请求资源可直接从CDN节点获取，无需回源，降低源站压力，提升了用户体验
- 不预热，这个时候CDN节点将请求源站点获取资源，即：==回源==
  - CDN节点上没有用户请求的资源或该资源的缓存已经过期了，此时需要CDN节点从原始服务器获取新的资源内容，这个过程就是回源

如果资源有更新，可用对其刷新，删除CDN节点上缓存的旧资源，并强制CDN节点回源站获取最新资源

- 几乎所有云厂商都提供CDN服务，且都具备缓存的刷新与预热



命中率与回源率是衡量CDN服务质量的重要指标
...
> GSLB：Global Server Load Balance，全局负载均衡，是CDN的大脑，负责多个CDN节点之间相互协作，最常用的是基于DNS的GSLB

CDN通过GSLB找到最合适的CDN节点

1. 浏览器向DNS服务器发送域名请求
2. DNS服务器根据CNAME（Canomical Name）别名记录向GSLB发送请求
3. GSLB返回性能最好(通常距离请求地址最近)的CDN节点（边缘服务器）的地址给浏览器
4. 浏览器直接访问指定的CDN
...
- GSLB根据请求IP地址、CDN节点状态等指标综合判断返回哪一个CDN节点地址



### 如何防止资源被盗刷？

如果我们的资源被其他用户或网站非法盗刷的话，会造成不小的开支

通常采用：Referer防盗链

- 具体就是根据Http请求头信息里面的Referer字段对请求进行限制
- 通过Referer字段获取到当前请求页面的来源页面的网站地址，这样可以确定请求是否来自合法的网站
- CDN服务商几乎都提供了比较基础的防盗链机制的

不过，如果站点的防盗链配置运行Referer为空时，隐藏Referer，可以绕开防盗链

此时，配合其他机制来确保静态资源不会被盗用

- 时间戳防盗链：时间戳防盗链加密的URL具有时效性，过期后就无法再被允许访问

- 时间戳防盗链的URL通常有2个参数

  - 签名字符串：一般是通过用户设定的加密字符串、请求路径、过期时间通过md5哈希算法取值的方式获得
  - 过期时间

- ~~~plain
  例如：http://cdn.wangsu.com/4/123.mp3?wsSecret=7r89wfifjsdfjfks&wsTime=1602938434
  ~~~

时间戳防盗链：实现简单，可靠性较高，推荐使用

- 大部分CDN服务提供商也都提供了开箱即用的时间戳防盗链机制

除了Referer防盗链、时间戳防盗链、还可以进行IP黑白名单配置、IP访问限制等机制来防止静态资源的盗刷



## 负载均衡

> 负载均衡是将用户的请求，分摊到不同的服务器上处理，以提高系统整体的并发处理能力以及可靠性

负载均衡可以由专门的软件、硬件来完成。硬件的性能是更好的，但是价格更够昂
...
- 主要运行/应用在：客户端请求和网关层之间，可以用软件或硬件实现

硬件负载均衡

- F5、A10、Array等
- 性能强、稳定，缺点：价格昂贵，例如F5基础款都要20w了
- 日常开发中，一般很难接触到硬件负载均衡
...
- LVS：Linux Virtual Server虚拟服务器，绝大多数公司用不上嘀，阿里、腾讯大厂才会有，价格几k~几w
- Nginx、HAproxy
- 虽然性能差一点，但是价格便宜啊
...
- 工作在OSI模型第四层，即传输层，通常是LVS负载均衡实现

七层负载均衡

- 工作在OSI模型第七层，即应用层
- 执行第七层负载均衡的设备，通常被称为：反向代理服务器，通常采用Nginx做七层负载均衡
- 反向代理：代理的是目标服务器，对外暴露的是反向代理服务器地址，隐藏了真实服务器IP地址



### 客户端负载均衡

客户端负载均衡主要应用于系统内部不同服务之间，可以使用现有的负载均衡组件实现

客户端负载均衡中，客户端自己会维护一份服务器列表，发送请求之前，客户端会根据负载均衡算法来选择一台服务器来处理请求

Java主流微服务框架Dubbo、Spring Cloud都内置了客户端负载均衡实现
...
代码质量、集群冗余设计、服务限流、降级&熔断、超时&重试

## 超时&重试

由于网络问题、系统服务bug、服务器宕机等，我们的系统或服务不可能保持时刻都是可用的

为了最大限度减小系统或服务出现故障后带来的影响，我们需要用到：超时和重试机制

- 平时涉及到远程调用的系统或服务都会应用超时和重试机制
- 尤其是微服务系统，设置超时和重试机制非常重要



超时机制

- 连接超时ConnectTimeout：客户端与服务端建立连接的最长等待时间

- ==读取超时ReadTimeout==：客户端等待服务端处理完请求的最长时间
...
- 太高或太低都没有意义
- 通常建议读取超时设置为1500ms，即1.5s。根据自己项目而定，没有银弹
...
一般情况下，重试和超时是一起使用的，指的是多次发送相同的请求，来避免瞬态故障和偶然性故障

重试的核心思想：通过消耗服务器资源来尽可能获得请求更大概率成功处理
...
- 通常重试的次数不宜过多，否则会对系统负责造成较大压力
- 通常建议3次，大部分更建议使用梯度间隔重试



重试幂等性问题

- 客户端等待服务端完成请求超时，但是此时服务端已经执行了请求，只是此刻的网络波动导致响应在发送给客户端的过程中出现延迟了
- 需要对接口进行幂等性判断
...
- while、for、递归都行
- 第三方框架：Spring Retry、Guave Retrying
