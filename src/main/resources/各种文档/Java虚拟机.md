...
2. JDBC、JNDI、SPI、Spring等等采用线程上下文类加载器来加载，并没有走双亲委派模型
   - 这是一种父加载器去请求子类加载器完成类加载的型为，已经违背了双亲委派模型的一般原则
   - Java中设计SPI的加载基本上都采用这种方式来完成
3. 为什么要打破双亲委派机制？
   - 如果有一个类，想要通过自定义的类加载器来加载这个类，而不是通过系统默认的类加载器，说白了就是不走双亲委派那一套流程，而是用自定义的类加载器去加载Class
4. 双亲委派机制是在ClassLoader类的loadClass()中实现的，如果我们不想使用系统自带的双亲委派模型，只需要重新实现ClassLoader的loadClass()方法即可，下面是ClassLoader中定义的loadClass()方法，里面实现了双亲委派机制
